...
const MONACO_JS: &[u8] = include_bytes!("assets/monaco.js");
const MONACO_CSS: &[u8] = include_bytes!("assets/monaco.css");

const MONACO_FILES: &[(&str, &[u8])] = &[
    ("monaco.js", MONACO_JS),
    ("monaco.css", MONACO_CSS),
];

pub fn make_app() -> Command {
    Command::new("mdbook-monaco")
        .version(crate_version!())
        .about("mdbook preprocessor to add monaco support")
        .subcommand(
...
fn handle_preprocessing() -> Result<(), Error> {
    ...
    if ctx.mdbook_version != mdbook::MDBOOK_VERSION {
        eprintln!(
            "Warning: The mdbook-monaco preprocessor was built against version \
    ...
    let processed_book = Monaco.run(&ctx, book)?;
...
fn handle_supports(sub_args: &ArgMatches) -> ! {
    ...
    let supported = Monaco.supports_renderer(renderer);
...
fn handle_install(sub_args: &ArgMatches) -> ! {
    ...
    for (name, content) in MONACO_FILES {
    ...
    log::info!("Files & configuration for mdbook-monaco are installed. You can start using it in your book.");
    let codeblock = r#"```monaco
id: editor1_chapter1
files:
    - name: "main.js"
    language: javascript
...
fn add_preprocessor(doc: &mut Document) {
    ...
    item["command"] = value("mdbook-monaco");
}
